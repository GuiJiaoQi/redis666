...
public class ClueController {
    ...
    public @ResponseBody Object saveCreateClue(Clue clue, HttpSession session){
        ...
        int num = clueService.saveCreate(clue);
        if (num>0){
            returnObject.setCode(Contants.RETURN_OBJECT_CODE_SUCCESS);
        }else {
            returnObject.setCode(Contants.RETURN_OBJECT_CODE_FAILO);
            returnObject.setMessage("线索保存失败");
        }
        return returnObject;
    }
    ...
    public @ResponseBody Object saveBundActivity(String clueId,String[] activityId){
        ...
        ReturnObject returnObject = new ReturnObject();
        int num = clueActivityRelationService.saveClueActivityRelationByList(relationList);
        //用传过来的activityId   取出关联的市场活动信息
    ...
    public @ResponseBody Object saveUnbundActivity(ClueActivityRelation relation){
        int num = clueActivityRelationService.deleteClueActivityRelationByClueIdActivityId(relation);
        ReturnObject returnObject = new ReturnObject();
        if (num > 0){
            returnObject.setCode(Contants.RETURN_OBJECT_CODE_SUCCESS);
        }else {
